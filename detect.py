...
def detect( source = None, weights = "", img_size = 640, no_save = False,
    ...
    # Directories
    # save_dir = increment_path(Path(projects) / name, exist_ok=False)  # increment run
    ...
    for det in pred:  # detections per image
        ...
        # p, s, im0 = path, '', im0s.copy() #for image only
        s = ''
        # p = Path(p)  # to Path
...
#     opt = parser.parse_args(["best.pt", "projects/yolov5_results/testimg"])
...
# image, labels = detect(source = image, weights="best.pt", img_size=640)
# for i in labels:
#     print(i)
